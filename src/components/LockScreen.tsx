...
export function LockScreen() {
  const { phase, setPhase, settings } = useOS();
  ...
  const getLockScreenBackground = () => {
    const wp = settings.wallpaper || '';
    if (isCustomWallpaper(wp)) {
      return `url(${wp}) center/cover no-repeat`;
    }
    return '#1a1a2e';
  };
  ...
  return (
    <div
      ...
      style={{
        background: getLockScreenBackground(),
    ...
    >
      ...
      <div style={{
        ...
        background: 'radial-gradient(ellipse at 30% 20%, rgba(0,120,212,0.15) 0%, transparent 50%), radial-gradient(ellipse at 70% 80%, rgba(83,52,131,0.2) 0%, transparent 50%)',
        pointerEvents: 'none',
      }} />
      {/* Clock panel */}
      <div
