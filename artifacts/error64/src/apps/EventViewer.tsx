...
const generateEvents = (): LogEvent[] => {
  ...
  return Array.from({ length: 150 }, (_, i) => ({
    ...
    message: ['The Error64 system has started successfully.','A user account password changed.','The system time has changed.','A service was installed in the system.','An account was successfully logged on.','Event log service started.','The browser service failed while processing the command.'][i % 7],
  }));
};
...
export function EventViewer() {
  ...
  const [log, setLog] = useState<'System' | 'Application' | 'Security'>('System');

  const filtered = ALL_EVENTS.filter(e => {
    if (filterLevel !== 'All' && e.level !== filterLevel) return false;
    if (filterSource && !e.source.toLowerCase().includes(filterSource.toLowerCase())) return false;
    if (log !== 'All' && e.category !== log) return false;
    return true;
  });

  const counts = {
    Information: ALL_EVENTS.filter(e => e.level === 'Information').length,
    Warning: ALL_EVENTS.filter(e => e.level === 'Warning').length,
    Error: ALL_EVENTS.filter(e => e.level === 'Error').length,
    Critical: ALL_EVENTS.filter(e => e.level === 'Critical').length,
  };
    ...
    <div style={{ display: 'flex', height: '100%', fontFamily: "'Segoe UI', sans-serif", fontSize: '13px' }}>
      ...
      <div style={{ width: '200px', background: '#f3f3f3', borderRight: '1px solid #ddd', flexShrink: 0, overflowY: 'auto' }}>
        ...
        <div style={{ paddingLeft: '12px' }}>
          ...
          {(['Application','Security','System'] as const).map(l => (
            <div key={l} onClick={() => setLog(l)}
              style={{ padding: '6px 20px', cursor: 'pointer', background: log === l ? '#d0e4ff' : 'transparent', borderLeft: log === l ? '3px solid #0078D4' : '3px solid transparent' }}
              onMouseEnter={e => { if (log !== l) e.currentTarget.style.background = '#e8e8e8'; }}
              onMouseLeave={e => { if (log !== l) e.currentTarget.style.background = ''; }}>
              {l}
            </div>
          ))}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '12px 16px', background: '#f9f9f9', borderBottom: '1px solid #ddd', display: 'flex', gap: '12px', flexShrink: 0 }}>
          ...
          {(Object.entries(counts) as [EventLevel, number][]).map(([level, count]) => (
            <div key={level} style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 10px', background: '#fff', border: '1px solid #ddd', cursor: 'pointer', background: filterLevel === level ? '#d0e4ff' : '#fff' }}
          ...
          <button onClick={() => setFilterLevel('All')} style={{ padding: '4px 12px', border: '1px solid #ccc', background: filterLevel === 'All' ? '#0078D4' : '#fff', color: filterLevel === 'All' ? '#fff' : '#333', cursor: 'pointer' }}>All</button>
          ...
          <div style={{ flex: 1, overflowY: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
              <thead style={{ position: 'sticky', top: 0, background: '#f3f3f3', zIndex: 1 }}>
                <tr>
                  {['Level','Date and Time','Source','Event ID','Category'].map(h => (
                    <th key={h} style={{ padding: '6px 10px', textAlign: 'left', fontWeight: 600, borderBottom: '2px solid #ddd', whiteSpace: 'nowrap' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map(event => (
                  <tr key={event.id}
                    ...
                    style={{ background: selected?.id === event.id ? '#d0e4ff' : 'transparent', cursor: 'pointer', borderBottom: '1px solid #f0f0f0' }}
                    onMouseEnter={e => { if (selected?.id !== event.id) e.currentTarget.style.background = '#e8f0fe'; }}
                    onMouseLeave={e => { if (selected?.id !== event.id) e.currentTarget.style.background = ''; }}>
